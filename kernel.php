<?php
define('iCMS',1);
define('URL','Location: http://'.$_SERVER['HTTP_HOST'].str_replace('//','/',dirname($_SERVER['PHP_SELF']).'/'));
...
if(isset($user) || isset($group) || isset($cfg) || isset($dinfo) || isset($lang) || isset($emodata)) exit('Acces Violation');
...
if(isset($_SESSION['uid']))
{
	...
	{
		db_read('login,pass,gid,lv,adm,lvis,pms','users','user',$xuid,' WHERE lv!=5 AND ID='.$xuid);
		if(isset($user[$xuid]))
		{
			#Test
...
elseif($_COOKIE[$cfg['c'].'login'])
{
	...
	{
		db_read('login,pass,gid,lv,adm,lvis,pms','users','user',$xuid[0],' WHERE lv!=5 AND ID='.$xuid[0]);
		if(isset($user[$xuid[0]]))
		{
			#Test
...
#Struktura kategorii
function CatStr($c=0,$d='')
{
 global $cfg,$lang,$dinfo,$nlang;
 if($cfg['cstr']!=1 || strpos($dinfo['opt'],'S')!==false || $_GET['om']==1) return false;
 $y='';
 echo '<div class="cs">'.(($d=='')?'':'<div style="float: right; clear: both">'.$d.'</div>').'<a href="?co=cats&amp;id='.$dinfo['type'].'">';
 switch($dinfo['type'])
 {
 ...
 if($dinfo['sc']!='P')
 {
  ...
  while($i!='P')
	{
   $cs=db_read('ID,name,sc','cats',0,'get',' WHERE ID='.$i.' && (access=1 || access="'.$nlang.'")');
   if(isset($cs))
   {
 ...
 echo $y.(($c)?' &raquo; <a href="?d='.$c.'">'.$dinfo['name'].'</a>':'').'</div>';
}
