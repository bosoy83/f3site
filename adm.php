...
if(LOGD==1)
{
	if($user[UID]['lv']!=2 && $user[UID]['lv']!=3) exit('Brak praw dostêpu!');
}
...
if($_GET['x'])
{
	...
	$x=str_replace('.','',$x);
	if(file_exists('./admin/inc/'.$x.'.php'))
	{
		require('./admin/inc/'.$x.'.php');
	}
	elseif(file_exists('./plugins/'.$a.'/admin2.php'))
	{
		require('./plugins/'.$a.'/admin2.php');
	}
	unset($x);
}
?>
