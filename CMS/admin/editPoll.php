...
{	
	...
	{
		$db->commit();
		$content->info('<center>'.$lang['saved'].'<br /><br /><a href="?a=editpoll">'.$lang['addpoll'].'</a> | <a href="index.php?co=poll&amp;id='.$id.'">'.$poll['name'].'</a></center>');
		return 1;
	}
...
<tr>
	<td><b>5. '.$lang['type'].':</b></td>
	<td>
		<input type="radio" name="x_t" value="1"'.(($poll['type']==1)?' checked="checked"':'').' /> '.$lang['max1'].' &nbsp;
		<input type="radio" name="x_t" value="2"'.(($poll['type']==2)?' checked="checked"':'').' /> '.$lang['maxd'].'
	</td>
</tr>';
	CloseBox();
