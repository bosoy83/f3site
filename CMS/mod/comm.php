...
if($_POST)
{
	#Dane
	$c = array(
		'name'  => Clean($_POST['c_n'],30,1),
		'guest' => (LOGD==1) ? 0 : 1
		'text'  => ($type) ? Clean($_POST['c_t'],0,1) : $_POST['c_t'],
	);
	...
	elseif(isset($_POST['save']))
	{
		...
		if($type) //NOWY KOM.
		{
			...
			else
			{
				...
				#KOD
				if($cfg['captcha']==1 && (empty($_POST['c_code']) || $_POST['c_code']!=$_SESSION['code']))
				{
					$error[]=$lang['c2'];
				}
			}
			...
			#Moderowaæ?
			$c_access=($cfg['moderate']!=1 || LEVEL>1 || Admit('CM'))?1:0;
		...
		if(!$error) //WSPÓLNE
		{
			...
			{
				$db->commit();
				$content->info($lang[ (($type && $c_access!=1)?'c6':'c7') ]); return;
			}
			catch(PDOException $e)
			{
				$content->info($lang['c10'].$e->errorCode());
			}
		}
		$db->rollBack();
	}
