...
if($_POST['sav'])
{
 ...
 else
 {
  ...
  db_q('UPDATE {pre}files SET cat="'.$xu_c.'", name="'.$xu_n.'", author="'.$xu_au.'", dsc="'.$xu_d.'", file="'.$xu_f.'", access="'.db_esc($xu_a).'", size="'.$xu_s.'", priotity="'.db_esc($xu_p).'", fulld="'.db_esc(TestForm($xu_fd,0,0,0)).'" WHERE ID='.$id);
  #Ilo¶æ plików
  if($file[0]!=$xu_c) { ChItmN($xu_c,'+1'); ChItmN($file[0],'-1'); }
  if($file[1]>$xu_a) ChItmN($xu_c,'+1');
  if($file[1]<$xu_a) ChItmN($xu_c,'-1');
 }
 Info('<div align="center">'.$lang['saved'].'<br /><br /><a href="?a=efile">'.$lang['addfile'].'</a></div>');
}
...
if(!$_POST['sav'])
{
 ...
 $ile=count($xcat);
 echo '<form action="adm.php?a=efile'.(($id=='new')?'':'&amp;id='.$id).'" method="post" enctype="multipart/form-data">';
 ...
 <tr>
  ...
  <td><select name="xu_p"><option value="1"'.(($file['priotity']==1)?' selected="selected"':'').'>'.$lang['high'].'</option><option value="2"'.(($file['priotity']==2 || $id=='new')?' selected="selected"':'').'>'.$lang['normal'].'</option><option value="3"'.(($file['priotity']==3)?' selected="selected"':'').'>'.$lang['low'].'</option></select></td>
 </tr>
 <tr>
