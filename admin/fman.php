...
if(ChPrv('FM2')) Info($lang['fmw'].'<br /><br /><center><form action="?x=fm&amp;act=up&amp;dir='.$dir.(($ff)?'&amp;ff='.$ff:'').'" method="post" enctype="multipart/form-data">'.$lang['fmupl'].': <input name="uplf" type="file" /> <input type="submit" value="OK" /> &nbsp;<input type="button" value="'.$lang['fmndir'].'" onclick="fm_nd(\'nd\',0)" /></form></center>');
cTable(((strlen($dir)>50)?'(...)'.substr($dir,-40):$dir),3);
echo '
<tr>
...
if($cp=opendir($dir)) {
 while(false !== ($dirq=readdir($cp))) {
  if($dirq!='.' && is_dir($dir.$dirq)) {
   if($dirq=='..' && $dir=='./') {  } else { echo '<tr><td align="center" style="padding: 3px"><img src="'.SCIMG.'" alt="[cat]" /></td><td><a href="?x=fm'.(($ff)?'&amp;ff='.$ff:'').'&amp;dir='.$dir.$dirq.'">'.(($dirq=='..')?'^ '.$lang['goup']:$dirq).'</a></td><td align="center">'.(($dirq!='..' && ChPrv('FM2'))?'<a href="javascript:fm_del(1,\''.$dirq.'\')">'.$lang['del'].'</a> &middot; <a href="javascript:fm_nd(\'rn\',\''.$dirq.'\')">'.$lang['fmchn'].'</a>':'').'</td></tr>'; }
  }
 }
 closedir($cp);
}
...
if($cp=opendir($dir)) {
 while(false !== ($dirq=readdir($cp))) {
  if(is_file($dir.$dirq)) {
   echo '<tr><td align="center" style="padding: 3px"><img src="'.IMGFILE.'" alt="[file]" /></td><td><a href="'.$dir.$dirq.'">'.$dirq.'</a></td><td align="center">'.((ChPrv('FM2'))?'<a href="javascript:fm_del(2,\''.$dirq.'\')">'.$lang['del'].'</a> &middot; <a href="javascript:fm_nd(\'rn\',\''.$dirq.'\')">'.$lang['fmchn'].'</a>':'').(($ff)?' &middot; <a href="javascript:opener.document.forms[0].'.$ff.'.value=\''.$dir.$dirq.'\'; opener.focus()">'.$lang['fins'].' &raquo;</a>':'').'</td></tr>';
  }
 }
 closedir($cp);
}
