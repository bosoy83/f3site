...
if($_POST['sav'])
{
 ...
 else
 {
  ...
  db_q('UPDATE {pre}imgs SET cat="'.$xu_c.'", name="'.$xu_n.'", author="'.$xu_au.'", dsc="'.$xu_d.'", file="'.$xu_f.'", filem="'.$xu_fm.'", access='.db_esc($_POST['xu_a']).', priority="'.db_esc($_POST['xu_p']).'", type="'.db_esc($_POST['xu_t']).'", size="'.db_esc($_POST['xu_s1']).'||'.db_esc($_POST['xu_s2']).'" WHERE ID='.$id);
  #Ilo¶æ artów
  if($img[0]!=$xu_c) { ChItmN($xu_c,'+1'); ChItmN($img[0],'-1'); }
  if($img[1]>$_POST['xu_a']) ChItmN($xu_c,'+1');
  if($img[1]<$_POST['xu_a']) ChItmN($xu_c,'-1');
 }
 Info('<center>'.$lang['saved'].'<br /><br /><a href="?a=editimg">'.$lang['addimg'].'</a></center>');
}
...
if(!$_POST['sav'])
{
 ...
 $ile=count($xcat);
 require('inc/btn.php');
 echo '<form action="adm.php?a=editimg'.(($id=='new')?'':'&amp;id='.$id).'" method="post">';
 ...
 <tr>
  ...
  <td><select name="xu_p"><option value="1"'.(($img['priority']==1)?' selected="selected"':'').'>'.$lang['high'].'</option><option value="2"'.(($img['priority']==2 || $id=='new')?' selected="selected"':'').'>'.$lang['normal'].'</option><option value="3"'.(($img['priority']==3)?' selected="selected"':'').'>'.$lang['low'].'</option></select></td>
 </tr>
 <tr>
