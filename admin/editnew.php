...
if($_POST['sav'])
{
 if($id=='new')
 {
  db_q('INSERT INTO {pre}news VALUES ("","'.$xu_c.'","'.$xu_n.'","'.db_esc($xu_txt).'","'.strftime('%Y-%m-%d %H:%M:%S').'","'.UID.'",'.(($_POST['xu_fn'])?1:2).',"'.$xu_i.'",'.(($_POST['xu_emo'])?1:2).',0,'.(($_POST['xu_br'])?1:2).',"'.db_esc($_POST['xu_a']).'")');
 ...
 Info('<div align="center">'.$lang['saved'].'<br /><br /><a href="?a=editnew">'.$lang['addnews'].'</a></div>');
}
#Podgl±d
if($_POST['preview'])
{
...
if(!$_POST['sav'])
{
 ...
 require('inc/btn.php');
 echo '<form action="adm.php?a=editnew'.(($id=='new')?'':'&amp;id='.$id).'" method="post">';
