...
if($_POST['sav'])
{
 ...
 else
 {
  ...
  db_q('UPDATE {pre}links SET cat="'.db_esc($xu_c).'", name="'.$xu_n.'", dsc="'.$xu_d.'", access="'.db_esc($xu_a).'", adr="'.db_esc($xu_adr).'", priority="'.db_esc($xu_p).'", nw='.(($xu_nw)?1:2).' WHERE ID='.$id);
  #Ilo¶æ linków
 ...
 Info('<center>'.$lang['saved'].'<br /><br /><a href="?a=editlink">'.$lang['addlink'].'</a></center>');
}
...
if(!$_POST['sav'])
{
 ...
 $ile=count($xcat);
 echo '<form action="adm.php?a=editlink'.(($id=='new')?'':'&amp;id='.$id).'" method="post">';
 ...
 <tr>
  ...
  <td><select name="xu_p"><option value="1"'.(($link['priority']==1)?' selected="selected"':'').'>'.$lang['high'].'</option><option value="2"'.(($link['priority']==2 || $id=='new')?' selected="selected"':'').'>'.$lang['normal'].'</option><option value="3"'.(($link['priority']==3)?' selected="selected"':'').'>'.$lang['low'].'</option></select></td>
 </tr>
 <tr>
